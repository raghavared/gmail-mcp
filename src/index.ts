...
const OAUTH_PATH = process.env.GMAIL_OAUTH_PATH || path.join(CONFIG_DIR, 'token.json');
const CREDENTIALS_PATH = process.env.GMAIL_CREDENTIALS_PATH || path.join(CONFIG_DIR, 'credentials.json');

const OAUTH_TOKENS = process.argv.indexOf('clinet_oauth_tokens') > -1 ? process.argv[process.argv.indexOf('clinet_oauth_tokens') + 1] : null;
const CREDENTIALS = process.argv.indexOf('credentials') > -1 ? process.argv[process.argv.indexOf('credentials') + 1] : null;
...
async function loadCredentials() {
    try {
        ...
        const localOAuthPath = path.join(process.cwd(), 'token.json');
        ...
        let tempOauthTokens = null;
        if (OAUTH_TOKENS) {
            try {tempOauthTokens = JSON.parse(OAUTH_TOKENS);} catch{}
        }
        if (CREDENTIALS) {
            try {tempOauthTokens = JSON.parse(CREDENTIALS);} catch{}
        }

        if (!fs.existsSync(OAUTH_PATH) &&  !tempOauthTokens) {
            console.error('Error: OAuth keys file not found. Please place token.json in current directory or', CONFIG_DIR);
            process.exit(1);
        }
        ...
        const keys = keysContent?.installed || keysContent?.web || tempOauthTokens;
        ...
        ? process.argv[3] 
        : "http://localhost:3000/v2/auth/google/callback";
        ...
        if (tempOauthTokens) {
            oauth2Client.setCredentials(tempOauthTokens);
        }
    } catch (error) {
...
async function authenticate() {
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        server.on('request', async (req, res) => {
            if (!req.url?.startsWith('/v2/auth/google/callback')) return;
